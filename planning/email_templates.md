...
**IMPORTANT:** These templates are treated as HTML. Use standard HTML tags for formatting (e.g., `<b>`, `<strong>`, `<br>`, `<a href="...">`). Newlines in the Google Doc are automatically converted to `<br>` tags.
...
<b>Requested by:</b> {{REQUESTED_BY_NAME}}<br>
...
<b>Project:</b> {{PROJECT_TITLE}}<br>
<b>Due Date:</b> {{DEADLINE}}<br>
<b>Days Remaining:</b> {{DAYS_UNTIL_DUE}}<br>
<b>Project ID:</b> {{PROJECT_ID}}
...
```html
• <strong>Project Name</strong> - Status changed to: <strong>New Status</strong><br>
...
<b>Category:</b> {{CATEGORY}}<br>
<b>Requested by:</b> {{REQUESTED_BY_NAME}}<br>
<b>Updated Deadline:</b> {{DEADLINE}}<br>
<b>Project ID:</b> {{PROJECT_ID}}
